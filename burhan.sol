...
contract SAAHypePenguins is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint public constant MAX_SUPPLY = 9000;

    uint public constant Whitelist_SUPPLY = 1000;
    //Maximum number of NFTs an address can mint
    uint public max_whitelistmint_allowed = 2;
    //Maximum number of NFTs an address can mint
    uint public max_publicmint_allowed = 3;

    uint public reserveAmount = 10;
    //Price of one NFT in presale
    uint public pricePresale = 0.20 ether;
  ...
  mapping(address => uint) public tokensMintedByAddress;
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI) ERC721("SAA Hype Penguins", "SHP"){
    ...
    **/
    ...
    function changeMaxWhitelistMintAllowed(uint _maxWhitelistMintAllowed) external onlyOwner {
        max_whitelistmint_allowed = _maxWhitelistMintAllowed;
    }

     function changeMaxMintAllowed(uint _maxPublicMintAllowed) external onlyOwner {
        max_publicmint_allowed = _maxPublicMintAllowed;
    }
  ...
  function Whitelistmint(uint256 _ammount) external payable nonReentrant {
        //Get the number of NFT sold
        uint numberNftSold = totalSupply();
          //The user can only mint max 2 NFTs
        require(_ammount <= max_whitelistmint_allowed, "You can't mint more than 2 tokens");
        //If Sale didn't start yet
        require(sellingStep == Steps.Whitelist, "Sorry, Whitelist has not started yet.");
        ...
        require(numberNftSold + _ammount <= Whitelist_SUPPLY, "Whitelistminting is almost done and we don't have enought NFTs left.");
        ...
        if(numberNftSold + _ammount == Whitelist_SUPPLY) {
    ...
    function Mint(uint256 _ammount) external payable nonReentrant {
        ...
        require(_ammount <= max_publicmint_allowed, "You can't mint more than 3 tokens");
        //If the user try to mint any non-existent token
        require(numberNftSold + Whitelist_SUPPLY + _ammount + reserveAmount <= MAX_SUPPLY, "Sale is almost done and we don't have enought NFTs left.");
        //Add the ammount of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _ammount;
        //If this account minted the last NFTs available
        if(numberNftSold + Whitelist_SUPPLY + _ammount + reserveAmount <= MAX_SUPPLY) {
            sellingStep = Steps.SoldOut;   
        }
        //Minting all the account NFTs
        for(uint i = 1 ; i <= _ammount ; i++) {
   ...
   function reserveSAA() public onlyOwner {        
        uint numberNftSold = totalSupply();
        uint i;
